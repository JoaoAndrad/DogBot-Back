...
try {
  ...
  console.warn('Falha ao carregar configuração de certificados:', e && e.message ? e.message : e);
}
...
function maskDatabaseUrl(url) {
  ...
  try {
    ...
    return url.replace(/(postgresql:\/\/)([^:\/]+):([^@]+)@/, (m, p1, user, pass) => {
...
async function recreatePrismaClient() {
  ...
  try {
    ...
    console.log('Diagnóstico de reconexão Prisma:', JSON.stringify(_lastPrismaDiag));
  } catch (e) {
    // Ignorar
  }
...
try {
  ...
  console.log('Diagnóstico de certificado Prisma/DB:', JSON.stringify(diag, null, 2));
} catch (e) {
  console.warn(
...
module.exports = { getPrisma, testConnection, recreatePrismaClient, getLastPrismaError };
